  ...
  <v-row no-gutters class="py-5 py-md-10 px-2 px-md-5" justify="center" style="min-height: 100%">
    <v-col cols="12" sm="10" md="12" lg="9" xl="8">
      ...
      <v-row no-gutters class="w-100 " align="center">
        ...
        <v-col cols="12" class="font-500 text-22px text-sm-h6 text-center px-sm-5 text-sm-start">{{ step == 1 ? 'Update Offer' : 'Review Updated Offer' }}</v-col>
      </v-row>
          ...
          <v-row no-gutters>
            <v-form @submit.prevent v-model="formValid" :key="step" class="w-100">
              ...
              <v-row no-gutters v-else class="w-100">
                <CustomOfferForm v-model:old="customOffer" v-model:new="computedOffer" v-model:bookings="bookings"
              ...
              <v-row no-gutters>
                ...
                <v-col cols="6" align="end">
                  <v-btn v-if="isVenueOwner || isVenueAdmin" type="submit" variant="flat" rounded="lg" color="secondary"
                    @click="handleSendOffer" :loading="processing">Update Offer</v-btn>
                </v-col>
              </v-row>
            </v-card>
          </v-form>
...
const handleSendOffer = async () => {
  ...
  let payload: any = {
    ...
    guests: guests,
    venue_computation: {
      subtotal: payment_computation.venue.subtotal,
      commission_fee: payment_computation.venue.commission_fee,
      cleaning_fee: payment_computation.venue.cleaning_fee,
...
const fetchOffer = async () => {
  try {
    ...
    if (data && data.value) {
      ...
      customOffer.value.date = responseData?.date?.date;
      customOffer.value.from = responseData?.date?.from;
      customOffer.value.to = responseData?.date.to;
      customOffer.value.guests = responseData?.guests;
      customOffer.value.payment_computation.user = responseData?.user_computation

      computedOffer.value.date = responseData?.date?.date;
      (computedOffer.value.guests = responseData?.guests),
        (computedOffer.value.payment_computation.user =
          responseData?.user_computation),
        (computedOffer.value.currency = responseData?.currency);
    }
  } catch (err) {
    console.log(err);
  }
