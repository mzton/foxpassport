  ...
  <v-row no-gutters justify="center">
    <v-form v-model="formValid" @submit.prevent ref="formRef" class="my-5 my-md-10 mx-3 mx-md-5 text-secondary w-100"
      ...
      <v-row no-gutters class="w-100 my-7 my-md-10">
        <v-col cols="12" class="text-24px font-weight-black">Create an account as user</v-col>
      ...
      <v-row no-gutters class="md-flex ga-5 flex-wrap w-100">
        <v-col>
          ...
          <v-text-field v-model="form.first_name" variant="outlined" placeholder="John" density="comfortable"
            rounded="lg" class="mt-1" :rules="[(v: string) => requiredInput(v)]"></v-text-field>
        </v-col>
        <v-col>
          ...
          <v-text-field v-model="form.last_name" variant="outlined" placeholder="Doe" density="comfortable" rounded="lg"
            class="mt-1" :rules="[(v: string) => requiredInput(v)]"></v-text-field>
        </v-col>
      </v-row>
      ...
      <v-row no-gutters class="d-flex ga-5 flex-wrap w-100 mt-2">
        <v-col cols="12">
          ...
          <v-text-field v-model="confirmPassword" variant="outlined" type="password" placeholder="+6 characters"
            density="comfortable" rounded="lg" class="mt-1" :rules="confirmPasswordRules"></v-text-field>
        </v-col>
      </v-row>
      ...
      <v-row no-gutters class="d-flex ga-5 flex-wrap w-100 mt-7">
        <v-col cols="12">
          <v-btn variant="flat" color="primary" type="submit" size="large" rounded="lg"
            class="mt-1 w-100 font-weight-black" :disabled="!formValid" @click="onSubmit" :loading="processing">Create Account</v-btn>
        </v-col>
      </v-row>
    </v-form>
...
const { validEmail, minPasswordLength, requiredInput } = useUtils();
