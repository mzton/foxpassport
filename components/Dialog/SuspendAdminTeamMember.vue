<template>
  <v-dialog v-model="adminSuspendDialog" :height="suspendConditional ? '230px' : '400px'" max-width="500">
    <v-card rounded="lg" height="100%" width="100%" style="overflow-y:hidden;">
      <v-row class="px-10 pt-7" style="max-height:75px;">
        ...
        <v-col cols="1">
          <v-row class="pt-1" style="max-height:20px;" @click.stop="adminSuspendDialog = false"
...
const { suspendAdminFunctionTeamMembers } = useTeamMembers()
const isSuspended = ref(null);
const adminSuspendDialog = defineModel();
...
async function suspendSelect(val) {
  ...
  try {
    const res = await suspendAdminFunctionTeamMembers(organization_member_id as string, suspensionCondition as string)
    if (res) {
      emit("suspending")
      adminSuspendDialog.value = false
    }
  }
  catch (err) {
